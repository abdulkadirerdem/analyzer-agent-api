...
async def upload_and_explain(
    ...
    try:
        ...
        # Fix the temporary file name in analysis response
        if "results" in analysis_response and isinstance(
            analysis_response["results"], dict
        ):
            temp_basename = os.path.basename(temp_path)
            if temp_basename in analysis_response["results"]:
                # Replace the temp file name with original file name
                result_data = analysis_response["results"][temp_basename]
                # Update the file name in the result
                result_data["file"] = file.filename
                # Replace the results with new key
                analysis_response["results"] = {file.filename: result_data}
                print(
                    f"Updated analysis response file name from {temp_basename} to {file.filename}"
                )
        ...
        code_data = analysis_response.get("results", {})
        print(
            f"Code data keys: {code_data.keys() if isinstance(code_data, dict) else 'Not a dictionary'}"
        )
        ...
        if code_data and isinstance(code_data, dict) and len(code_data) > 0:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                if len(code_data) == 1:
                    ...
                    # Ensure the file field is set correctly
                ...
                try:
                    with open(analysis_path, "r") as verify:
                        ...
                        print(
                            f"Verification: Temp file contains {len(verify_data)} entries"
                        )
                        for key in verify_data.keys():
                            print(f"- Verify key: {key}")
                            if (
                                isinstance(verify_data[key], dict)
                                and "functions" in verify_data[key]
                            ):
                                print(
                                    f"  - Contains {len(verify_data[key]['functions'])} functions"
                                )
                                print(
                                    f"  - File field: {verify_data[key].get('file', 'not set')}"
                                )
                except Exception as e:
            ...
            try:
                ...
                # Fix any remaining issues with file names in the result
                if "file" in result and (
                    not result["file"] or result["file"] != file_name
                ):
                    print(
                        f"Fixing file name in result: {result['file']} -> {file_name}"
                    )
                    result["file"] = file_name

                # Check and fix markdown header if necessary
                if "markdown" in result and result["markdown"]:
                    temp_basename = os.path.basename(temp_path)
                    if result["markdown"].startswith(
                        "# 📄 Documentation for ``"
                    ) or result["markdown"].startswith(
                        f"# 📄 Documentation for `{temp_basename}`"
                    ):
                        # Replace with correct file name
                        new_markdown = (
                            f"# 📄 Documentation for `{file_name}`"
                            + result["markdown"].split("\n", 1)[1]
                        )
                        result["markdown"] = new_markdown
                        print(
                            f"Fixed markdown header to use correct file name: {file_name}"
                        )

                print("----- Upload endpoint completed -----\n")
                return result
            finally:
...
async def analyze_and_explain(
    ...
    try:
        ...
        analysis_response = analyze_file(analyze_request)
        print(
            f"Analysis response keys: {analysis_response.keys() if isinstance(analysis_response, dict) else 'Not a dictionary'}"
        )
        ...
        if code_data and isinstance(code_data, dict) and len(code_data) > 0:
            ...
            try:
                ...
                print(
                    f"Explanation result keys: {explanation_result.keys() if isinstance(explanation_result, dict) else 'Not a dictionary'}"
                )

                # Fix any remaining issues with file names in the result
                if "file" in explanation_result and (
                    not explanation_result["file"]
                    or explanation_result["file"] != file_name
                ):
                    print(
                        f"Fixing file name in explanation result: {explanation_result['file']} -> {file_name}"
                    )
                    explanation_result["file"] = file_name

                # Check and fix markdown header if necessary
                if "markdown" in explanation_result and explanation_result["markdown"]:
                    if explanation_result["markdown"].startswith(
                        "# 📄 Documentation for ``"
                    ) or explanation_result["markdown"].startswith(
                        f"# 📄 Documentation for `{temp_basename}`"
                    ):
                        # Replace with correct file name
                        new_markdown = (
                            f"# 📄 Documentation for `{file_name}`"
                            + explanation_result["markdown"].split("\n", 1)[1]
                        )
                        explanation_result["markdown"] = new_markdown
                        print(
                            f"Fixed markdown header to use correct file name: {file_name}"
                        )
                ...
                if "markdown" in explanation_result:
                    print(
                        f"Markdown content (first 100 chars): {explanation_result['markdown'][:100]}"
                    )
                else:
                ...
                if "important_functions" in explanation_result:
                    print(
                        f"Important functions: {len(explanation_result['important_functions'])}"
                    )
                ...
                print(
                    f"Explanation structure: {list(result['explanation'].keys()) if isinstance(result['explanation'], dict) else 'Not a dictionary'}"
                )
...
async def debug_format(file: UploadFile = File(...)):
    """
